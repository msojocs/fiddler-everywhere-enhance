  ...
  const regExpI18n = [
    {
      ...
      condition: (node) =>
        node.parentNode &&
        node.parentNode.className === "content content-un-upload",
      en: ($0, $1) =>
        `There are ${$1} videos not submitted in the local browser`,
    },
    {
      regExp: /还可以添加(\d+)个标签/,
      condition: (node) =>
        node.parentNode && node.parentNode.className === "tag-last-wrp",
